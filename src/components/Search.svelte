  ...
  const fakeResult = [
    {
      ...
      excerpt: "Para ver os resultados, execute o projecto em produção",
    },
  ];
...
<div
  id="search-panel"
  class="float-panel closed search-panel absolute md:w-[30rem]
...
>
  ...
  {#each result as item}
    ...
    >
      ...
      >
        {item.meta.title}<slot name="arrow-icon" />
      </div>
